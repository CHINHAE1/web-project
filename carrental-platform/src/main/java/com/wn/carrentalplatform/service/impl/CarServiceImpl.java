...
//        PageInfo<Car> pageInfo = new PageInfo<>(cars);
    ...
    @Override
    public int updateCarByCarnumber(CarVo carVo) {
        // 复制属性
        Car car = new Car();
        BeanUtils.copyProperties(carVo, car);
        // 执行修改
        return carMapper.updateCarByCarnumber(car);
    }

    @Override
    public int deleteCarByCarnumber(CarVo carVo) {
        // 执行删除
        return carMapper.deleteCarByCarnumber(carVo.getCarnumber());
    }
