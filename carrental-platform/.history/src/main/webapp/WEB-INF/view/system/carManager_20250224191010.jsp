...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'upload'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '/car_rental/car/queryCars' //数据接口
            ...
            , cols: [[   //列表数据
                ...
                , {field: 'carimg', title: '缩略图', align: 'center', width: '80', templet:
                        // '<div><img src="{{d.carimg}}" width=40 height=40/></div>'
                        function(d) {
            // 修改这里的图片路径拼接方式
            return '<img src="/car_rental/upload/' + d.carimg + '" width=40 height=40/>';
        }
                }
...
//            alert(params);
            tableIns.reload({
                url: "/car_rental/car/queryCars?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(carTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.carnumber + '】这个车辆么？', function (index) {
                    //向服务端发送删除指令
                    $.post("/car_rental/car/deleteCar", {carnumber: data.carnumber}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddCar() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#dataFrm")[0].reset();
                    //设置默认图片
                    $("#showCarImg").attr("src", "http://localhost:80/wyz.jpg");
                    $("#carimg").val("http://localhost:80/wyz.jpg");
                    url = "/car_rental/car/addCar";
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些车辆么？', function (index) {
                //向服务端发送删除指令
                $.post("/car_rental/car/deleteBatchCar.action", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
